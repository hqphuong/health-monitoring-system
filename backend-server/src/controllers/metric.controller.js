...
export const syncHealthData = async (req, res) => {
    console.log("==================================================");
    console.log("🚀 [SYNC START] Nhận request đồng bộ dữ liệu...");
    ...
    try {
        ...
        // --- BƯỚC 1: LƯU DATABASE (Giữ nguyên logic của Duy) ---
        const chunkSize = 100;
        ...
        console.log(`✅ [DB] Đã đồng bộ xong ${totalProcessed} mẫu dữ liệu.`);

        // --- BƯỚC 2: KIỂM TRA NHỊP TIM NGUY HIỂM (Giữ nguyên logic của Duy) ---
        const incomingHeartRates = data
            .filter(item => item.heart_rate !== undefined && item.heart_rate !== null)
            .map(item => item.heart_rate);
        
        const maxHR = incomingHeartRates.length > 0 ? Math.max(...incomingHeartRates) : 0;

        if (maxHR > 150) {
            console.log(`🚨 [CRITICAL] Nhịp tim cao: ${maxHR} BPM. Đang kích hoạt cảnh báo...`);
            ...
            if (io) {
                ...
                if (primaryContact) {
                    ...
                    io.to(relativeRoom).emit("relative_warning", {
                        type: "PATIENT_SOS",
                        patient_name: "Người thân của bạn", 
                        heart_rate: maxHR,
...
export const getHealthMetrics = async (req, res) => {
    try {
        ...
        // SỬA LỖI TRUY VẤN THÁNG 5: Mở rộng range để tránh lệch múi giờ Server/Client
        if (range === 'day') {
            startDate.setHours(now.getHours() - 36); // Lấy 36 tiếng gần nhất thay vì 24h
        } else if (range === 'week') {
            startDate.setDate(now.getDate() - 8); // Lấy 8 ngày
        } else if (range === 'month') {
            startDate.setMonth(now.getMonth() - 1);
            startDate.setDate(startDate.getDate() - 1); // Trừ thêm 1 ngày cho chắc
        }
        ...
        const getLocalDate = (date) => {
            ...
            d.setHours(d.getHours() + 7); // Force sang GMT+7 (Việt Nam)
        ...
        const groups = metrics.reduce((acc, curr) => {
            ...
            // SỬA LỖI SPO2: Đảm bảo lọc bỏ các giá trị rỗng/0 trước khi push
            if (curr.blood_oxygen && curr.blood_oxygen > 0) g.spo2_samples.push(curr.blood_oxygen);
